...
(*let rec eclate(t,i,j,couleur) = if donne_1(t,i+1,j)=0 then eclate(t,i+1,j,couleur) else remplace(t,i,j,couleur);;*)
(*let rec eclate(t,i,j,couleur) = if donne_1(t,i+1,j)=0 then t else remplace(t,i,j,couleur);;*)
affiche(tableau,5,3);;
eclate(tableau,0,0,8);;
