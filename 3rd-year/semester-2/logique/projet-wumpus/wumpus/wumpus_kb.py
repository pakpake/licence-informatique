...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 06:29:44 2021
...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        if not tvec[i]:
            perceptions[i] = "~" + perceptions[i]
        perceptions[i] += str(t)

    axiom_str = " & ".join(perceptions)
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    for (i,j) in voisins:
    	if (xmin <= i <= xmax) and (ymin <= j <= ymax):
    		puits.append(pit_str(i,j))
    puits.append(pit_str(x,y))
    axiom_str = breeze_str(x,y) + " <=> (" + (" | ").join(puits) + ")"
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = stench_str(x,y) + " <=> (" + (" | ").join(Wumpus) + ")"
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    axiom_str = ''
    grid_tuple = [(i,j) for i in range(xmin, xmax + 1) for j in range(ymin, ymax + 1)]
    axioms = []
    ...
    for g in grid_tuple:
        ...
        for h in grid_tuple:
            ...
            if g != h:
                no_wumpus.append("~" + wumpus_str(h[0], h[1]))

        axioms.append("(" + wumpus_str(g[0],g[1]) + " >> ("+ (" & ".join(no_wumpus)) +"))")
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    "*** YOUR CODE HERE ***"
    # ****
    liste = [state_loc_str(i,j,t) for i in range(xmin, xmax + 1) for j in range(ymin, ymax + 1)]
    liste = [a for a in liste if a != state_loc_str(xi,yi,t)]
    axiom_str = state_loc_str(xi,yi,t) + " & ~(" + (" | ").join(liste) + ")"
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    if x + 1 <= xmax:
        clauses.append('(({0} & {1} & {2}))'.format(state_loc_str(x+1,y,t), state_heading_east_str(t), action_forward_str(t)))
    if x - 1 >= xmin:
        clauses.append('(({0} & {1} & {2}))'.format(state_loc_str(x-1,y,t), state_heading_west_str(t), action_forward_str(t)))
    if y + 1 <= ymax:
        clauses.append('(({0} & {1} & {2}))'.format(state_loc_str(x,y+1,t), state_heading_north_str(t), action_forward_str(t)))
    if y - 1 >= ymin:
        clauses.append('(({0} & {1} & {2}))'.format(state_loc_str(x,y+1,t), state_heading_south_str(t), action_forward_str(t)))
    clauses.append('({0} & (~{1} | {2} | {3} | {4} | {5} | {6}))'.format(state_loc_str(x,y,t),action_forward_str(t),percept_bump_str(t+1),action_grab_str(t),action_shoot_str(t),action_turn_left_str(t),action_turn_right_str(t)))

    axiom_str = state_loc_str(x,y,t+1) + " <=> " + (' | '.join(clauses))
    """
    depls = [
    	"({0} & (~{1} | {2} | {3} | {4} | {5} | {6}))"
    	.format(
    ...
    voisins = [(((x-1),y),"E"),((x,(y-1)),"N"),(((x+1),y),"W"),((x,(y+1)),"S")]

    for ((i,j),depl) in voisins:
        if xmin <= i <= xmax and ymin <= j <= ymax:
    ...
    axiom_str = "{0} <=> ({1})".format(state_loc_str(x,y,t+1)," | ".join(depls))
    return axiom_str
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    # axiom_str = '{0} <=> ({1} & ~{2})'.format(state_wumpus_alive_str(t+1), state_wumpus_alive_str(t), percept_scream_str(t))
    axiom_str = state_wumpus_alive_str(t+1) + " <=> (" + state_wumpus_alive_str(t) + "& ~" + percept_scream_str(t) + ")"
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = "{0} <=> ({1} | {2} | {3})".format(state_heading_north_str(t+1),actions,left,right)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    axiom_str = "{0} <=> ({1} | {2} | {3})".format(state_heading_east_str(t+1),actions,left,right)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    axiom_str = "{0} <=> ({1} | {2} | {3})".format(state_heading_south_str(t+1),actions,left,right)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    axiom_str = "{0} <=> ({1} | {2} | {3})".format(state_heading_west_str(t+1),actions,left,right)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    for action in actions:
        ...
        axioms.append("(" + action + str(t) + " <=> (" + (" & ".join(other)) + "))")
