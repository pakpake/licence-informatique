...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 06:34:05 2021
...
import os
# os.chdir('/Users/user/Desktop/Wumpus')
os.chdir('./')

from wumpus_agent import *
# from time import perf_counter as clock    # not available after python3.7
...
def with_manual_kb_program(agent):
    ...
    def show_commands():
        ...
        # TODO: pb parenthèse avant le .format, à mettre à la fin
        print ("     {0}".format(', '.join(map(lambda a: '\'{0}\''.format(a), actions))))
        print ("   Enter {0} to get this command info" \
              .format(' or '.join(map(lambda a: '\'{0}\''.format(a), helping))))
        print ("   Enter {0} to stop playing" \
...
def run_minisat_test():
    ...
    print ("Running simple CryptoMiniSat test:")
    t = 1
    failed = []
