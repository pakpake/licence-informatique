...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 06:31:47 2021
...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        "*** YOUR CODE HERE ***"
        distance = [ manhattan_distance_with_heading(node.state, goal) for goal in self.goals  ]
        return min(distance)
    ...
    def actions(self, state):
        ...
        explorerloc = self.allowed
        for WumpusLoc in possibleWumpusLocations:
            if (yesForward[0],yesForward[1]) in explorerloc:
                movef = min(movef,manhattan_distance_with_heading(yesForward,WumpusLoc))
            movel = min(movel, manhattan_distance_with_heading(yesleft,WumpusLoc))
            moveri = min(moveri, manhattan_distance_with_heading(yesright,WumpusLoc))
...
class PlanShotProblem(search.Problem):
    ...
    def actions(self, state):
        ...
        for ExplorerLoc in explorerLocations:
            for WumpusLoc in possibleWumpusLocations:
                if (WumpusLoc[0] == ExplorerLoc[0]):
                    if ExplorerLoc[1] > WumpusLoc[1]:				
                        shotSpots.append((ExplorerLoc[0],ExplorerLoc[1],2))
                    if ExplorerLoc[1] < WumpusLoc[1]:				
                        shotSpots.append((ExplorerLoc[0],ExplorerLoc[1],0))
                elif (WumpusLoc[1] == ExplorerLoc[1]):
                    if ExplorerLoc[0] > WumpusLoc[0]:				
                        shotSpots.append((ExplorerLoc[0],ExplorerLoc[1],1))
                    if ExplorerLoc[0] < WumpusLoc[0]:				
                        shotSpots.append((ExplorerLoc[0],ExplorerLoc[1],3))
        ...
        possibleWumpusLocations = self.goals
        explorerloc = self.allowed
        for WumpusLoc in shotSpots:
            if (yesForward[0],yesForward[1]) in explorerLocations:
                movef = min(movef,manhattan_distance_with_heading(yesForward,WumpusLoc))
            movel = min(movel, manhattan_distance_with_heading(yesleft,WumpusLoc))
            moveri = min(moveri, manhattan_distance_with_heading(yesright,WumpusLoc))
        if movef <= min(movel, moveri):
    ...
    def goal_test(self, state):
        ...
        for location in exploc:
            for aim in possibleWumpusLocations:
                if location[0] == aim[0] and location[1] > aim[1]:
                    shotSpots.append((location[0],location[1],2))
                elif location[0] == aim[0] and location[1] < aim[1]:
                    shotSpots.append((location[0],location[1],0))
                elif location[0] > aim[0] and location[1] == aim[1]:
                    shotSpots.append((location[0],location[1],1))
                elif location[0] < aim[0] and location[1] == aim[1]:
                    shotSpots.append((location[0],location[1],3))
        return state in shotSpots
