...
def getAuthor1(fpdf):
    ...
    """
    Recupere les auteurs du fichier dans les métadonnées de ce dernier
    Fonction qui appelle la commande pdfinfo pour avoir les différentes informations du fichier
    """
   ...
   #permettant de recuieillir les informations du subprocess
    ...
    #appel de la commande pdfinfo/grep/cut en subprocess.Popen, créant ainsi un object permettant la manipulation des entrées/sorties des commandes
    pdfinfo=subprocess.Popen(["/usr/bin/pdfinfo",fpdf],stdout=subprocess.PIPE)
    
    
    grep=subprocess.Popen(["grep","-e",'Author'],stdin=pdfinfo.stdout,stdout=subprocess.PIPE)
   
    
    cut=subprocess.Popen(["cut","-d:","-f","2"], stdin=grep.stdout,stdout=subprocess.PIPE, universal_newlines=True)
    
    #On lit la sortie standart de cut et on sépare les differents élements avec "\n"
    ...
    #on enlève tout les espaces devant et derrière
...
def getAuthor2(fpdf):
	...
	try:
		...
		print(author)
		print("\n")
		author = [x for x in author if not any(c.isdigit() for c in x) and not re.search(r'([\w.-]+)@([\w.-]+)', x)and not "Universit" in x and not "Google" in x and not "Depart" in x and not "Polytechnique" in x and not "Abstract" in x and x!='' and x[0].isupper()]
...
def getAuthor3(fpdf):
	"""
	Dans le cas où on arrive pas à récuperer le titre
	"""
	subprocess.run(["/usr/bin/pdftotext","-raw",fpdf])
	ftxt =getBasename(fpdf)+".txt"
	try:
		...
		author=author.split("\n")
		#On récupère tout les élements de la liste qui ne répondent à ces critères (si l'élément ne contient pas de chiffres, s'il ne contient pas d'emails, s'il n'a pas "Université"/ "Google" / "Abstract" dans l"élément, s'il n'est pas vide et si le premier caractère n'est pas une minuscule
		...
		#On sépare les differents auteurs par un ;
		othor='; '.join(author)
		return othor
	finally:
		f1.close()
