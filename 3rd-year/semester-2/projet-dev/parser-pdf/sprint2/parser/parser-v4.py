...
"""
Programme parser pdf vers txt v4, qui enregistre dans un sous-dossier avec même noms de fichier
@author: Team 
...
def verifPDF(fichier):
    """Prend un fichier en parametre
       l'ouvre et lit la premiere ligne
       pour verifier que c'est bien "%PDF-" """
    
    ## verification = open(fichier,"rb")
    ## debut = verification.readline()

    #debut = open(fichier,"rb").readline()
    #return b'%PDF-' in debut
    
    #Ouverture du fichier en binaire et lecture de la premiere ligne seulement
    #Recherche de la chaine de caracteres "%PDF-" qui est presente quand le fichier est un PDF 
