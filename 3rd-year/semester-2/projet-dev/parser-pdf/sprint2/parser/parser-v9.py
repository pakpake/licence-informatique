...
def getAuthor1(fpdf):
    ...
    if open(getAuthor.ftxt,'r').read().find(author[0])==-1:
        author[0]=""
    return author[0]

def getAuthor2(fpdf):
    """ Dans le cas ou les informations ne sont pas recuperes par getAuthor1 avec pdfinfo """
    # creation du fichier tmp.txt qui contient le texte brut
    ftxt = getAuthor.ftxt
    try:
...
def getAuthor3(fpdf):
    """ Dans le cas ou on arrive pas a recuperer les auteurs avec getAuthor2 """
    ftxt = getAuthor.ftxt
    try:
        f1 = open(ftxt,"r")
        author = ""
...
def getAuthor(fpdf):
    ...
    # on convertit en texte dans tous les cas, 
    # car pdfinfo ne donne pas toujours le bon autheur
    subprocess.run(["/usr/bin/pdftotext","-l","1","-raw",fpdf])
    # on modifie dynamiquement le namespace getAuthor pour rendre accessible ftxt a l'exterieur
    getAuthor.ftxt = getBasename(fpdf)+".txt"
    b = getAuthor1(fpdf)
    # si on ne trouve pas d'auteurs via pdfinfo,
    # ou si on en trouve, mais qu'ils ne sont pas visible dans le pdf
    if b=="":
        b=getAuthor2(fpdf)
#    if b=="":
#        b=getAuthor3(fpdf)
    # suppression du fichier txt temporaire
    #os.remove(getAuthor.ftxt)
    return b
...
if __name__ == "__main__":
    ...
    if len(listeNePasParser) != 0:
        ...
        resultError = resultName+"/List_Of_Non_PDF_Files.txt"
