...
if __name__ == "__main__":
    ...
    i=0
    lpdf=[]
    for file in glob.glob(nomDossier+"*.pdf"):
        print("["+str(i)+"]","---",file)
        lpdf.append(os.path.basename(file))
        i+=1
    ...
    if sys.argv[1] == "-a" or sys.argv[1] ==  "-x":
        for k in listeFinale:
            ...
            # conclusion
            fichier.write("      <conclusion> "+k[5]+" </conclusion>\n")
            # discussion
            fichier.write("      <discussion> "+k[6]+" </discussion>\n")
            # biblio
            fichier.write("      <biblio> "+k[7]+" </biblio>\n")
            fichier.write("</article>\n")
            # on ferme le fichier courant
            fichier.close()
