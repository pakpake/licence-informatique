...
def getAffli(fpdf):
	...
	try:
		...
		#On cherche les différentes affiliations
		a=[]
...
def getAdress(fpdf):
	...
	try:
		...
		#On corrige quelques petites choses..
		i=0
		while i<len(author):
			if "and" in author[i]:
				author[i]=re.sub(r'and',r'',author[i])
		...
		for i in range(1, len(author)):
			...
			if "chemin" in author[i]:
				if "Univers" in author[i] :
					b=author[i].split(",")
					a.append(b[1])
				else:
					a.append(author[i])
				continue
			if re.search(r'\d{3}',author[i]) :
				if not re.search('[1-2][0-9]{3}',author[i]):
					if author[i] not in a: 
						a.append(author[i])
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "-a" or sys.argv[1] ==  "-x":
        for k in listeFinale:
            ...
            # Adresses
            fichier.write("     <adresses> "" </adresses>\n")
            # Affliliations
            fichier.write("     <affiliations> Affiliations : "+k[9]+" / Adresses:"+k[10]+" </affiliations>\n")
            # abstract
