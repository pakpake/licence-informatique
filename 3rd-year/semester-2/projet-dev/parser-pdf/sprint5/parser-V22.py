...
def getAbstract1(fpdf):
    ...
    subprocess.run(["/usr/bin/pdftotext","-l","2",fpdf,"tmp.txt"])
    ftxt = "tmp.txt"
    try:
        # open file
        f1 = open(ftxt,"r")
...
def getAbstract2(fpdf):
    ...
    # creation du fichier tmp.txt qui contient le texte brut
    #subprocess.run(["/usr/bin/pdftotext","-l","2","-raw",fpdf,"tmp.txt"])
    ftxt = getBasename(fpdf)+".txt"
    try:
        # open file
        f1 = open(ftxt,"r")
...
def getAuthor3(fpdf):
    ...
    try:
        ...
        while not re.search(r'a.b.s.t.r.a.c.t',line,re.IGNORECASE) and not re.search(r'abstract',line,re.IGNORECASE):
            line=f1.readline()
            author+=line
        
        author=author.split("\n")
        # On recupere tous les elements de la liste qui ne repondent a ces criteres :
        ...
        # s'il n'est pas vide et si le premier caractere n'est pas une minuscule
        author = [x for x in author if not any(c.isdigit() for c in x) and not re.search(r'([\w.-]+)@([\w.-]+)', x)and not "Universit" in x and not "Google" in x and not re.search(r'abstract',x,re.IGNORECASE) and x!='' and x[0].isupper()]
        # On separe les differents auteurs par un ;
        othor='; '.join(author)
        return othor
    finally:
...
def getAffilEtAdr(fpdf):
    ...
    try:
        ...
        for i in range(len(abs)):
            if abs[i] != "" and abs[i] != " ":
                premier=abs[i]+" " + abs[i+1]
                break
        
        line=f1.readline()

        #On copie ce qu'il y a entre le titre et l'abstract
        ...
        i = len(contenu)
        """
        Ici, on s'arrête à la premiere occurence des cas suivants pour l'Affiliation, sinon on continue jusqu'à la fin du tableau
        """
        while i>0 and not "niversit" in contenu[0] and not "nstitut" in contenu[0] and not "aboratoire" in contenu[0] and not "partement" in contenu[0] and not "partment" in contenu[0] and not "cole" in contenu[0] and not "Google" in contenu[0]:
        	
        	del contenu[0]
        	i=i-1
        """
     	Ici, on supprime tout le reste, ces' à dire tout ce qui n'est pas une adresse ou 
     	une affiliation
     	"""
        qAfficher=""
        for ligne in contenu:
            if ligne not in titre and "@" not in ligne and ligne not in authors and ligne not in abs and "article" not in ligne and not re.search(r'([\w.-]+)\.([\w.-]+)', ligne) and not re.search(r'([\w.-]+) \d{2}, \d{4}', ligne) and not re.search(r'\\,\[', ligne) and ligne not in qAfficher:
                qAfficher+= ligne + " "
            if premier in ligne:
                break
...
def traite1fichier(fpdf):
    ...
    z = getAffilEtAdr(fpdf)
    # et on le met dans la liste
    liste.append(z)

    #Affiliation et adresse
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "-a" or sys.argv[1] ==  "-x":
        for k in listeFinale:
            ...
            # Affliliations
            fichier.write("      <affiliations>"+k[9]+" </affiliations>\n")
            # abstract
