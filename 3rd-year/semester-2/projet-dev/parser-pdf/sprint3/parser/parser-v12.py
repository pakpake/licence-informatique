...
def getAuthor2(fpdf):
    ...
    try:
        ...
        auteurEmail = [x for x in author if not any(c.isdigit() for c in x) and not "Universit" in x and not "Google" in x and not "Abstract" in x and x!='' and x[0].isupper()]
        # récupération email
        email = ""
        for i in auteurEmail:
...
def getAuthor(fpdf):
    ...
    e1 = e2 = e3 = e = ""
    (b,e1) = getAuthor1(fpdf)
    if b=="":
        (b,e2) = getAuthor2(fpdf)
    if b=="":
        (b,e3) = getAuthor3(fpdf)
    if e1 != "":
        e = e1
    elif e2 != "":
        e = e2
