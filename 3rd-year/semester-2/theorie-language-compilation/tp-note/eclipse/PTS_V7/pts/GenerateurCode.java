...
public class GenerateurCode {
  ...
  public LinkedList<Instruction> parcours(Noeud node, 
    ...
    case CASE_STATEMENT: {	//TODO: case
        String vrai1 = newLabel();
        String vrai2 = newLabel();
        String faux1 = newLabel();
        code.addAll(parcours(node.fils[0], true, null, vrai1, faux1));	// si je laisse next_label à la place
        																// de faux1, le code renvoit toujours 0.
        code.add(new Instruction(vrai1,CodeOperation.NOP,null,null));
        code.addAll(parcours(node.fils[1], false, next_label, null, null));
        // 
        code.addAll(parcours(node.fils[2], true, null, vrai2, faux1));
        code.add(new Instruction(vrai2,CodeOperation.NOP,null,null));
        code.addAll(parcours(node.fils[3], false, next_label, null, null));
        code.add(new Instruction(null,CodeOperation.JMP,null,next_label));
        code.add(new Instruction(faux1,CodeOperation.NOP,null,null));
        code.addAll(parcours(node.fils[4], false, next_label, null, null));
    	break;
    }
    case BOOLEAN_VALUE:
