...
public class AnalyseurTypes {
  ...
  public Synthesized parcours(Noeud node) {
    ...
    case FOR_STATEMENT: // TODO: for
    	node.symbole = parcours(node.fils[0]).symbol;
    	node.symbole.setType(Type.INTEGER);
    	if (!parcours(node.fils[1]).type.isInteger())
    		Erreur.compilation(Messages.TYPE_FOR_INIT,node.ligne);
    	if (!parcours(node.fils[2]).type.isInteger())
    		Erreur.compilation(Messages.TYPE_FOR_LAST,node.ligne);
    	parcours(node.fils[3]);
    	break;
    }
